...
-- Insert default trading configs for the 3 channels
INSERT INTO public.trading_config (
  ...
  trailing_stop_pct,
  time_based_sell_enabled,
  time_based_sell_minutes,
  priority,
  enabled,
  notes
) VALUES 
  -- memecoin-alpha: HIGH allocation, standard stop loss
  ('memecoin-alpha', 0.5, -25.00, 100.00, 200.00, true, true, 15.00, false, NULL, 'high', true, 'High conviction plays - larger allocation'),
  
  -- memecoin-chat: LOW allocation, tighter stop loss (volatile)
  ('memecoin-chat', 0.1, -15.00, 50.00, 100.00, true, true, 10.00, true, 30, 'low', true, 'Volatile chat - small allocation, quick exits'),
  
  -- under-100k-chat: LOW allocation, tighter stop loss (volatile microcaps)
  ('under-100k', 0.1, -20.00, 75.00, 150.00, true, true, 12.00, true, 45, 'low', true, 'Microcaps - small allocation, moderate exits')
...
-- Create index for faster position monitoring
CREATE INDEX IF NOT EXISTS idx_trades_auto_sell ON public.trades(status, auto_sell_enabled) 
WHERE status IN ('bought', 'partial_tp1');
